class Scissors extends Hand {
  ...
  checkSnip(ellipseX, ellipseY) {
    // let hit = collidePointTriangle(ellipseX, ellipseY, scissors.average.base.x, scissors.average.base.y, scissors.index.tip.x, scissors.index.tip.y, scissors.middle.tip.x, scissors.middle.tip.y);
    let hit = collidePointTriangle(ellipseX, ellipseY, this.midB.i2m.x, this.midB.i2m.y, this.index.tip.x, this.index.tip.y, this.middle.tip.x, this.middle.tip.y);
    if (hit) {
      let targeted = true;
      setTimeout( () => { targeted = false; }, 1000);
      if (targeted && (this.dist.B2B.i2m <= (this.dist.T2T.i2m * 1.2)) && hit) { return true; }
    }
  }
